...
input=(Path(__file__).parent/"02_input.txt").read_text().split("\n")
...
def puzzle_1(input):
    ...
    while len(burn_down_list) > 0:
        ...
        max_count, char=max_count.split(" ")
        ...
        if char_count > max_count:
            is_bad=True
        print([min_count, max_count, char, password, password.count(char), is_bad])
        if not is_bad:
            good.append(i)
    return len(good)
...
def puzzle_2(input):
    ...
    while len(burn_down_list) > 0:
        ...
        p1=int(p1)
        p2=int(p2)
        p1_char=password[p1-1]
        p2_char=password[p2-1]

        is_good=False
        for p in [p1_char, p2_char]:
            if p==char:
                is_good=True
                if p1_char == p2_char:
                    is_good=False

        print([p1, p2, char, password, p1_char, p2_char, is_good])
        if is_good:
            good.append(i)
