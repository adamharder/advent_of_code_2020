...
input=(Path(__file__).parent/"03_input.txt").read_text().split("\n")
input=[i.strip() for i in input if i.strip() != ""]

# print(input)


def puzzle_1(input, right=3, down=1):

    trees=0
    y_pos=0
    x_pos=0
    while y_pos<len(input):

        # print(f"{y_pos}, {x_pos}")
        line=input[y_pos]
        # print(f"   {line}")
        while x_pos >= len(line):  # repeat the line as necessary
            line=line+line
        # print(f"   {   line}")
        ...
        # print(f'   {line[0:x_pos]}{tree_char}{line[x_pos+1:]} {is_tree}')
        y_pos+=down
        x_pos+=right
...
def puzzle_2(input):
    ...
    for i in [
        ...
        r= r * puzzle_1(input=input, right=i[0], down=i[1]) 
    return r
